...
export function Hero({ copy, media }: { copy?: SectionCopy; media: MediaMap }) {
  if (copy?.visible === false) return null;

  const heroBackground = getMedia(media, "hero.bg");
  const eyebrow = (copy?.eyebrow || "Streetlight Automation | Est. Dec 2024").split("|").map((item) => item.trim()).filter(Boolean);
  const primaryHref = copy?.ctaHref || "#contact";
      ...
      <div className="relative z-10 mx-auto grid max-w-[980px] items-center px-6 py-14 sm:py-16 lg:min-h-[calc(100vh-80px)] lg:py-20">
        <div className="max-w-3xl">
          ...
          <div className="mt-7 flex flex-wrap gap-3 sm:mt-9 sm:gap-4">
            <a className="ves-button focus-ring bg-ves-lime text-ves-black hover:bg-ves-paper" href={primaryHref}>
