...
export function ProjectCard({ project, index, large = false, compact = false }: { project: Project; index: number; large?: boolean; compact?: boolean }) {
  return (
    <article className={`group overflow-hidden rounded border border-ves-leaf/20 bg-ves-cream shadow-soft transition hover:-translate-y-1 hover:border-ves-leaf ${large ? "lg:grid lg:grid-cols-[1fr_1.05fr]" : compact ? "md:grid md:grid-cols-[0.72fr_1fr]" : ""}`}>
