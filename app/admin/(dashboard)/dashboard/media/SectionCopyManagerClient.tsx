...
const sectionLabels: Record<string, string> = {
  ...
  "home.about": "About",
  "home.about.mission": "About Mission Line",
  "home.about.card.deployment": "About Card: Deployment",
  "home.about.card.controlBoxes": "About Card: Control Boxes",
  "home.about.card.billReduction": "About Card: Bill Reduction",
  "home.about.card.warranty": "About Card: Warranty",
...
export function SectionCopyManagerClient({ initialCopies }: { initialCopies: AdminSectionCopy[] }) {
  ...
  const aboutCopies = aboutSectionKeys.map((key) => initialCopies.find((copy) => copy.sectionKey === key)).filter((copy): copy is AdminSectionCopy => Boolean(copy));
  const sectionCopies = initialCopies.filter((copy) => !heroStatSectionKeys.includes(copy.sectionKey) && !aboutSectionKeys.includes(copy.sectionKey));
  ...
  return (
    <section className="mt-10">
      ...
      {aboutCopies.length > 0 ? <AboutSectionEditor initialCopies={aboutCopies} /> : null}
...
function AboutSectionEditor({ initialCopies }: { initialCopies: AdminSectionCopy[] }) {
      ...
      const payloads = aboutSectionKeys
        ...
        .map((sectionKey) => {
          ...
          return adminFetch(`/api/admin/section-copy/${encodeURIComponent(sectionKey)}`, {
            ...
            return payload.copy as AdminSectionCopy;
          });
        });
  ...
  return (
    <form className="mt-5 rounded border border-ves-leaf/20 bg-white p-5 shadow-sm" onSubmit={save}>
      <div className="flex flex-wrap items-start justify-between gap-4 border-b border-ves-leaf/15 pb-4">
        <div>
          ...
          <h3 className="mt-2 text-xl font-black text-slate-950">Main about text and four visible cards</h3>
          <p className="mt-2 max-w-3xl text-sm font-semibold leading-6 text-slate-500">
            This edits the exact homepage block beginning with “Practical infrastructure technology for local governments and rural communities.”
          </p>
        </div>
        ...
        {aboutCardSectionKeys.map((sectionKey) => {
          ...
          return (
            <label className="grid gap-2 rounded border border-slate-200 bg-slate-50 p-4 text-sm font-black text-slate-700" key={sectionKey}>
              <span>{sectionLabels[sectionKey]?.replace("About Card: ", "") ?? "Card"}</span>
